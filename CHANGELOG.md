...
### Changed
- **BREAKING: Oracle events API now returns teams as objects with id and name**
  - Before: `teams: ["Team A", "Team B"]`
  - After: `teams: [{ id: "team_a", name: "Team A" }, { id: "team_b", name: "Team B" }]`
  - Clients should use `id` for `predicateValue` and `name` for display
  - This fixes the API design inconsistency where display values were returned but technical values were expected
  - WSIM/mwsim should update to use `team.id` instead of `team.name` for predicateValue
